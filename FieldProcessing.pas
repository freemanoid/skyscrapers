...
var
  itrVis, itr: shortint;
begin
  for itrVis:= 1 to FieldSize do
  begin
    //we can do not to check max and min visibility cases because we have special procedure for it
    if (VisibilityArray[Field.visLeft][itrVis] > 1) and (VisibilityArray[Field.visLeft][itrVis] < FieldSize) then
    begin
      if SearchOnRowInterval (UnitsArray, FieldSize, itrVis, 1, FieldSize) <> 0 then
      for itr:= FieldSize downto 1 do  
      begin
      ...
      VisibilityArray[Field.visLeft][itrVis]:= 0;
  end;
end;
